...
In ultimo è possibile dimostrare che la classe dei linguaggi regolari è chiusa rispetto alle tre operazioni regolari (vedi Sipser cap.1).

### Equivalenza tra i DFA e gli NFA
...
Sia $N{=}(Q,\Sigma,\delta,q_0,F)$ l'NFA di partenza che riconosce un linguaggio $A$ e voglio costruire un DFA $M{=}(Q\prime,\Sigma\prime,\delta\prime,q_0^\prime,F\prime)$ equivalente. Inizialmente considero il caso più semplice in cui $N$ non ha $\epsilon$-archi

\begin{enumerate}[itemsep=-7pt]
\item Ogni stato di $M$ è un insieme di stati di $N$.
$$Q\prime = \mathcal{P}(Q)$$

\item Se $R$ è uno stato di $N$, esso è anche un insieme di stati di $N$. Quando $M$ legge un simbolo $w$ nello stato $R$, mostra dove $w$ porta ogni stato in $R$ e poichè da ogni stato si può andare in un insieme di stati, ne prendiamo l'unione.
...
\item $M$ accetta se uno dei possivili stati in cui $N$ potrebbe essere in quel punto è uno stato accettante.
$$F\prime = \{R{\in}Q\prime\, |\, \text{$R$ contiene uno stato accetante di $N$}\}$$
\end{enumerate}

Adesso considero anche gli $\epsilon$-archi e, per ogni stato $R$ di $M$, definisco $E(R)$ come la collezione di stati che possono essere raggiunti dagli elementi di $R$ proseguendo solo con $\epsilon$-archi (includendo gli stessi elementi di $R$). Per $R{\subseteq}Q$, sarà
