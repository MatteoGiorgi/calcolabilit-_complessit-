# Algoritmi e macchine \hfill \textcolor{gray75}{\faGear}
...
\begin{equation*}
\texttt{> a b ... a } \underset{\text{\faHandPointerO}}{\texttt{b}} \texttt{ b \# \#}
\end{equation*}
...
\boxed{\qquad u \qquad}\ \underset{\text{\faHandPointerO}}{\sigma}\ \boxed{\qquad v \qquad}
\end{gather*}
...
Consideriamo una macchina $M$ che calcoli la somma di due numeri naturali scritti in notazione unaria usando il simbolo `|` e separati dal simbolo `+`. La seguente successione di configurazioni rappresenterà la computazione $(q_0, \underset{\text{\faHandPointerO}}{\texttt{>}} \texttt{|+||\#}){\rightarrow^*_M}(h, \texttt{>} \texttt{|||} \underset{\text{\faHandPointerO}}{\texttt{\#}})$
\begin{align*}
(q_0, \underset{\text{\faHandPointerO}}{\texttt{>}} \texttt{|+||\#}) &\rightarrow (q_0, \texttt{>} \underset{\text{\faHandPointerO}}{\texttt{|}} \texttt{+||})\\
                                                                     &\rightarrow (q_0, \texttt{>} \texttt{|} \underset{\text{\faHandPointerO}}{\texttt{+}} \texttt{||})\\
                                                                     &\rightarrow (q_1, \texttt{>} \texttt{||} \underset{\text{\faHandPointerO}}{\texttt{|}} \texttt{|})\\
                                                                     &\rightarrow (q_1, \texttt{>} \texttt{|||} \underset{\text{\faHandPointerO}}{\texttt{|}})\\
                                                                     &\rightarrow (q_1, \texttt{>} \texttt{||||} \underset{\text{\faHandPointerO}}{\texttt{\#}})\\
                                                                     &\rightarrow (q_2, \texttt{>} \texttt{|||} \underset{\text{\faHandPointerO}}{\texttt{|}})\\
                                                                     &\rightarrow (h, \texttt{>} \texttt{|||} \underset{\text{\faHandPointerO}}{\texttt{\#}})
\end{align*}
