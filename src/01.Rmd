# Algoritmi e macchine \hfill \faGear
...
\begin{equation*}
\texttt{> a b ... a } \underset{\uparrow}{\texttt{a}} \texttt{ b \# \#}
\end{equation*}
...
\item $\delta{\subseteq}(Q{\times}\Sigma){\times}(Q{\cup}{h}){\times}\Sigma{\times}\{\Lleftarrow,\Rrightarrow,\equiv\}$ è la relazione di transizione, tale che il carattere puntato dal cursore non possa mai trovarsi a sinistra di \textnormal{\ttfamily >}
...
\boxed{\qquad u \qquad}\ \underset{\uparrow}{\sigma}\ \boxed{\qquad v \qquad}
\end{gather*}
...
Ne viene intuitivo dedurre che $\Sigma^*$ rappresenti l'insieme di cardinalità infinita delle stringhe generabili dalla \textit{giustapposizione} ($\circ$) dei caratteri dell'alfabeto della macchina $\Sigma$, mentre $\Sigma^F$ è più facile a scriversi che a dirsi: $\Sigma^*{\circ}(\Sigma{\backslash}\{\textnormal{\texttt{\#}}\}){\cup}\{\epsilon\}$[^4].
...
\begin{gather*}
(q_0, \underset{\uparrow}{\texttt{>}} \texttt{|+||\#})
\rightarrow (q_0, \texttt{>} \underset{\uparrow}{\texttt{|}} \texttt{+||})
\rightarrow (q_0, \texttt{>} \texttt{|} \underset{\uparrow}{\texttt{+}} \texttt{||})
\rightarrow (q_1, \texttt{>} \texttt{||} \underset{\uparrow}{\texttt{|}} \texttt{|})\rightarrow\\
\rightarrow (q_1, \texttt{>} \texttt{|||} \underset{\uparrow}{\texttt{|}})
\rightarrow (q_1, \texttt{>} \texttt{||||} \underset{\uparrow}{\texttt{\#}})
\rightarrow (q_2, \texttt{>} \texttt{|||} \underset{\uparrow}{\texttt{|}})
\rightarrow (h, \texttt{>} \texttt{|||} \underset{\uparrow}{\texttt{\#}})
\end{gather*}
rappresenterà la computazione $(q_0, \underset{\uparrow}{\texttt{>}} \texttt{|+||\#}){\rightarrow^*_M}(h, \texttt{>} \texttt{|||} \underset{\uparrow}{\texttt{\#}})$, mentre la funzione $\delta$ può essere tabellata come segue
...
Qui invece gli stati[^7] della macchina
...
[^7]: Ho rappresentato gli stati della MdT come un automa finito non deterministico solo per praticità, questo non implica nessuna equivalenza tra macchine e automi.
